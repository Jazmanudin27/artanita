...
import { submitCheckout } from '../services/presensiService';
...
export const CheckoutScreen: React.FC = () => {
  ...
  const todayPresensi = route.params?.todayPresensi;
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const response = await submitCheckout(user.kode_guru, locationText, imageUri || undefined);
      Alert.alert('Sukses 🎉', response.message || 'Presensi check-out berhasil dicatat!', [
        { text: 'OK', onPress: () => navigation.goBack() },
      ]);
    } catch (error: any) {
      console.error(error);
      const msg = error.response?.data?.message || 'Gagal mengirim presensi check-out.';
      Alert.alert('Gagal Check-Out', msg);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.headerCard}>
        <Ionicons name="log-out" size={32} color="#f97316" />
        <View style={styles.headerTextContainer}>
          <Text style={styles.title}>Absen Pulang (Check-Out)</Text>
          <Text style={styles.subtitle}>
            Jam Masuk Hari Ini: {todayPresensi?.jam_in || 'Belum Absen Masuk'}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.card}>
        ...
        >
          {locationLoading ? (
            ...
            <>
              <Ionicons name="refresh-outline" size={16} color="#f97316" style={{ marginRight: 6 }} />
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          ...
          <Text style={styles.cardTitle}>Foto Selfie Check-Out (Opsional)</Text>
        </View>
      ...
      >
        {submitting ? (
          ...
          <>
            ...
            <Text style={styles.submitBtnText}>KIRIM ABSEN PULANG</Text>
          </>
        )}
      </TouchableOpacity>
    </ScrollView>
