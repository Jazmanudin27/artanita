@if(count($siswa) > 0)
    <div class="siswa-absensi-container">
        @foreach ($siswa as $s)
            @php
                $bgClass = $s->status == 'A' ? 'bg-danger-subtle text-danger border-danger' : 
                          ($s->status == 'S' ? 'bg-warning-subtle text-dark border-warning' : 
                          ($s->status == 'I' ? 'bg-info-subtle text-info border-info' : 'bg-success-subtle text-success border-success'));
                ...
                <div class="card-body p-3 d-flex align-items-center justify-content-between">
                    ...
                    <div class="status-select-wrapper">
                        <select class="form-control status-pill-select" data-kelas="{{ $s->kode_kelas }}" data-id="{{ $s->kode_siswa }}"
                                style="background-color: {{ $bgColor }}; color: {{ $textColor }}; border: 1.5 solid {{ $textColor }}; border-radius: 10px; font-weight: 700; font-size: 0.82rem; padding: 6px 12px; cursor: pointer; transition: all 0.2s ease;">
...
@else
    <div class="card border-0 shadow-sm text-center p-4" style="border-radius: 16px; background: #ffffff;">
        <ion-icon name="people-outline" style="font-size: 48px; color: #94a3b8;"></ion-icon>
        <div class="font-weight-bold text-dark mt-2" style="font-size: 0.95rem;">Tidak ada data siswa</div>
        <div class="text-muted" style="font-size: 0.8rem;">Pilih tanggal atau kelas lain untuk melihat data presensi.</div>
    </div>
@endif
            ...
            $.ajax({
                ...
                success: function() {
                    ...
                    if (typeof Swal !== 'undefined') {
                        ...
                        Toast.fire({
                            icon: 'success',
                            title: 'Status presensi tersimpan'
                        });
                    }
                },
            });
