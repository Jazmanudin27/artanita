        ...
        <tbody>
            @foreach ($laporan as $l)
                @php
                    ...
                    for ($i = 1; $i <= 12; $i++) {
                        $sakit[$i] = DB::table('absensi_mapel')
                            ->where('status', 'S')
                    ...
                    $totizin = DB::table('absensi_mapel')
                        ->where('status', 'I')
                        ->whereRaw("YEAR(tanggal) = '$tahun'")
                        ->where('kode_siswa', $l->kode_siswa)
                        ->where('kode_mapel', $kode_mapel)
                    ...
                    $totsakit = DB::table('absensi_mapel')
                        ->where('status', 'S')
                        ->whereRaw("YEAR(tanggal) = '$tahun'")
                        ->where('kode_siswa', $l->kode_siswa)
                        ->where('kode_mapel', $kode_mapel)
                    ...
                    $totalfa = DB::table('absensi_mapel')
                        ->where('status', 'A')
                        ->whereRaw("YEAR(tanggal) = '$tahun'")
                        ->where('kode_siswa', $l->kode_siswa)
                        ->where('kode_mapel', $kode_mapel)
